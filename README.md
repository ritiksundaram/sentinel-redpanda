...
agent.py    ->  reads anomalies, decides severity + action, logs it to agent.audit
approve.py  ->  reads decisions, asks a human y/n for risky ones, writes agent.actions

Five topics: telemetry.raw, telemetry.clean, telemetry.anomalies,
agent.audit, agent.actions.
Run it
Start Redpanda:
...
The "AI" in agent.py is a mock right now (llm_triage) — just threshold logic
with the same shape as a real model call, so dropping in an actual LLM is a small
change. Wanted to focus on the pipeline, not the model.
